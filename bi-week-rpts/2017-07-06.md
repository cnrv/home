...
### RISC-V Linux第三版
SiFive的Palmer 上周提交了第三版的RISC-V [linux支持](https://lkml.org/lkml/2017/6/28/734)。

### SeL4 on SMP
Hesham最近完成了SeL4的SMP支持。他在Spike中使用了2到9个核来测试。这是他的博客和一些关于sel4 on RISC-V的[介绍。](http://heshamelmatary.blogspot.com.au/2017/06/update-sel4risc-v-smp-support-sel4.html)
...
很多人正在讨论合并AUIPC和JALR指令(instruction fusion)来实现直接长跳转的可能性。ARM的AArch64指令集已经使用了指令合并技术，比如Cortex-A72。此外，这个讨论串解释了RISC-V User level 2.2 specification对支持返回地址栈(return address stack，分支预测的技术之一)的[设计考量](https://groups.google.com/a/groups.riscv.org/d/msgid/isa-dev/22781.41698.49397.828281%40KAMacBookPro2016.local)
