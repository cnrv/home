...
可望于八月底RISC-V GNU工具链有完整的支持，RV32E是RISC-V中针对嵌入式平台所制定的指令集，
与RV32I的主要差异在于RV32E仅有16个寄存器，以满足嵌入式平台对于功耗与面积成本的需求，
并在规格中更进一步限定RV32E仅支援M，A以及C延展。

除了指令集规格上的差异外，RV32E与RV32I的ABI(Application Binary Interface)也有部份差异，
例如RV32I中可用a0-a7共八个寄存器传递参数，在RV32E则只有a0-a5可用来传递参数，
并且针对嵌入式平台的使用情境，栈的对齐要求也从原本的16位元组对齐变成仅要求4位元组对齐即可，
以此来缩减记忆体用量。
