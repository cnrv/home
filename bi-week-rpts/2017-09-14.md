...
Andes Technology (晶心科技) 最近釋出他們開發中的 RISC-V LLVM port, 共釋出 LLVM, Clang 以及 compiler-rt,
該版本基於 LLVM 5.0 開發, 目前支援 RV32IMAC 以及 RV64IMA 的編譯, 且已通過多項開源及商業测试套件, 具有相當程度的穩定度與可靠性,
他們表示希望透過這次的開源能夠加速 RISC-V 社群上 LLVM 相關開發及避免各方的重複開發, 並且及早讓社群有個穩定可用的 LLVM Toolchain.
