...
其实，从2016年CCS安全会议的一篇文章来看，使用Intel处理器的Linux系统早就被攻破，完全不需要利用分支预测(的侧信道)。
...
攻击者通过寻找自己的数据空间在内核空间的对应映射，便能够做到内核太代码注入，用以劫持内核 [[Gruss2014](#Gruss2016), [Kemerlis2014](#Kemerlis2014)]。
