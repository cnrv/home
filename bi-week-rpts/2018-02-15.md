...
如果该扩展不被支持，即仍然使用以往用户和内核共享空间的页表方式，SUATP和SATP的值严格相等，都指向共享页表的基址。
如果使用该扩展，SATP指向内核页表的基址，SUATP指向用户页表的基址。内核页表和用户页表使用不同的ASID防止TLB刷新。
...
具体细节请参见PR:https://github.com/riscv/riscv-isa-manual/pull/128/files
