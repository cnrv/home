...
在RV64中，加载一个立即数或符号地址比RV32复杂得多。`lui`在RV64中同样会有符号拓展的问题。我在[rv8的一个网址](https://rv8.io/asm)上参考关于汇编器的资料时发现：
...
>    0:	00032537          	lui     a0,0x32 （测试生成的结果是lui a0,0x33 可能是输错了）
>    4:	bfb50513          	addi    a0,a0,-1029
>    8:	00e51513          	slli    a0,a0,0xe
>    c:	abe50513          	addi    a0,a0,-1346
> ```

此处加载32位的立即数时，`li`被展开为4条指令，而不是上面的2条指令。通过riscv64-unknown-elf-gcc编译以下指令可以发现，被加载的寄存器的高32位会被置1。

```assembly
	.text
	.global f
	.equ const,0xcafebabe
f:
	lui a1,%hi(const)
	addi a1,a1,%lo(const)
```

至于在RV64中具体如何处理超过32位的立即数以及上述的最高位为1的32位立即数，目前并没有找到相关资料，如果有大佬知道这些细节，感谢补充。
...
- [rv8 RISC-V Assembler Reference](https://rv8.io/asm)
